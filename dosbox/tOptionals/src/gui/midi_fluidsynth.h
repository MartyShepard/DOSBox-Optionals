...
public:
	...
	inline bool FluidSampleSF2File (const std::string& name) {
		struct stat buffer;   
	...
	bool Open(const char * conf) {
		...
		if (!FluidSampleSF2File(soundfont.c_str())){
			
			
			if ( strlen(soundfont.c_str()) == 0 ){
				E_Exit("Check Your Config\n\nMidi Output Fluidsynth Enabled and no Soundfont Selected");		
			}			
			
			E_Exit("Check Your Config\n\nMidi Output Fluidsynth Enabled and Soundfont not found:\n"
			       "\"%s\"",soundfont.c_str());
		}
