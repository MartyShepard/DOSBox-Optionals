...
bool MidiHandler_mt32::Open(const char *conf) {
	...
	if (MT32EMU_RC_ADDED_CONTROL_ROM != service->addROMFile(pathName)) {
		...
		if (MT32EMU_RC_ADDED_CONTROL_ROM != service->addROMFile(pathName)) {
			delete service;
			service = NULL;
			E_Exit("Midi Output MT32 Enabled and Missing Control ROM file\nMissing: CM32L_CONTROL.ROM or MT32_CONTROL.ROM");
			return false;
		}
	}
	...
	if (MT32EMU_RC_ADDED_PCM_ROM != service->addROMFile(pathName)) {
		...
		if (MT32EMU_RC_ADDED_PCM_ROM != service->addROMFile(pathName)) {
			delete service;
			service = NULL;
			E_Exit("Midi Output MT32 Enabled and Missing PCM ROM file\nMissing: CM32L_PCM.ROM or MT32_PCM.ROM");
			return false;
		}
	}
