...
static void MPU401_Reset(void) {
	...
	mpu.state.data_onoff=-1; //=0 SVN r4192
							 //Ignore unrequested data in intelligent mode;
							 //fixes Roland sound in Krusty's Fun House.
	mpu.state.command_byte=0;
...
public:
	MPU401(Section* configuration):Module_base(configuration){
		...
		if(_stricmp(s_mpu,"none") == 0) return;
		if(_stricmp(s_mpu,"off") == 0) return;
		if(_stricmp(s_mpu,"false") == 0) return;
...
*/
		...
		if(_stricmp(s_mpu,"uart") == 0) mpu.intelligent = false;
	...
	~MPU401(){
		...
		if(_stricmp(section->Get_string("mpu401"),"intelligent")) return;
