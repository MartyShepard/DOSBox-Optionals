 ...
 */
 ...
 /* All extended work are from John Campbell. Many thanks for this you are great :) */
...
#if defined(_MSC_VER)
	/* 
		const fmath::local::uint64_t to double possible loss of data
	*/
...
struct GFGus {
	/*
		gRegSelectData:
		what is read back from 3X3. not necessarily the index selected, but
	...
	*/
    ...
    void updateMasterVolume(void)
	{
		...
		{
			/*
				allow some amplification but don't let it overflow
			*/
			vol = 120;
		}
...
public:
	...
	*/
	...
	INLINE void WaveUpdate(void)
	{
		...
		if ((WaveCtrl & (WCTRL_STOP | WCTRL_STOPPED)) == 0/*voice is running*/)
		{
			...
			*/
			...
			{
				/* 
					unsigned int subtract, mask, compare. will miss start pointer if WaveStart <= WaveAdd. This bug
					is deliberate, accurate to real GUS hardware, do not fix.
				*/				
				WaveAddr 	-= WaveAdd;
			...
			if (endcondition)
			{
				...
				if ((RampCtrl & WCTRL_16BIT/*roll over*/) && !(WaveCtrl & WCTRL_LOOP))
				{
					/* "3.11. Rollover feature
						...
						Each voice has a 'rollover' feature that allows an application to be notified when a voice's
						playback position passes over a particular place in DRAM.  This is very useful for getting
						seamless digital audio playback.  Basically, the GF1 will generate an IRQ when a voice's current
						position is  equal to the end position.  However, instead of stopping or looping back to the
						start position, the voice will continue playing in the same direction.  This means that there
						will be no pause (or gap) in the playback.  Note that this feature is enabled/disabled through
						the voice's VOLUME control register (since there are no more bits available in the voice control
						registers).  A voice's loop enable bit takes precedence over the rollover.  This means that if a
						voice's loop enable is on, it will loop when it hits the end position, regardless of the state of
						the rollover enable."
						 
						Despite the confusing description above, that means that looping takes precedence over rollover.
						If not looping, then rollover means to fire the IRQ but keep moving. If looping, then fire IRQ and
						carry out loop behavior. Gravis Ultrasound Windows 3.1 drivers expect this behavior, else Windows
						WAVE output will not work correctly.
					*/
				}
				else {
	...
	INLINE void UpdateVolumes(void)
	{
		...
		templeft &= ~(templeft >> 31);						/* <- NTS:
																This is a rather elaborate way to clamp negative
																values to zero using negate and sign extend
															*/
												
		Bit32s tempright= (Bit32s)RampVol - (Bit32s)PanRight;
		tempright &= ~(tempright >> 31);					/* <- NTS:
																This is a rather elaborate way to clamp negative
																values to zero using negate and sign extend
															*/
	...
	INLINE void RampUpdate(void)
	{
		...
		{
			/*
				if the ramping is turned off, then don't change the ramp
			*/
			return; 										
		}
    ...
    void generateSamples(Bit32s* stream, Bit32u len)
	{
        ...
        /* NTS:
			The GUS is *always* rendering the audio sample at the current position,
...
static void GUSReset(void)
{
	...
	/* 
		...
		Global Data High (3X5) is either an 8-bit transfer for one of the GF registers or the high part of a 16-bit
		wide register with 8-bit I/O.
	
		Prior to 2015/12/29 DOSBox and DOSBox-X contained a programming error here where reset and master IRQ enable
		were handled from the LOWER 8 bits, when the code should have been checking the UPPER 8 bits. Programming
		error #2 was the mis-interpetation of bit 0 (bit 8 of the gRegData). According to the SDK, clearing bit 0
		triggers RESET, setting bit 0 starts the card running again. The original code had it backwards.
	*/
	...
	*/
	...
	if ( (GUS_reset_reg & 1) == 0 || (p_GUS_reset_reg & 1) == 0 )
	{
		...
		/*
			IRQ enable resets, unless user specified we force it on		 
		*/
		myGUS.irqenabled = myGUS.force_master_irq_enable; 
		GUS_reset_reg 	&= 1;
	}
		...
		if (irqstat != 0)
		{
			...
			*/
			if (gus_prev_effective_irqstat == 0)
			{
				...
				{
					LOG(LOG_MISC,LOG_WARN)("GUS warning: Both IRQs set to the same signal line WITHOUT combining! "
...
static Bit16u ExecuteReadRegister(void)
{
	...
	switch (myGUS.gRegSelect)
	{
		case 0x8E:							/* read active channel register */
			/* NTS:
				The GUS SDK documents the active channel count as bits 5-0,
				which is wrong. it's bits 4-0. bits 7-5 are always 1 on real 
				Hardware.		
			*/
		...
		case 0x41: 								/* Dma control register - read acknowledges DMA IRQ */	
			if (dma_enable_on_dma_control_polling)
			{
				...
				{
					LOG(LOG_MISC,LOG_NORMAL)("GUS: As instructed, switching on DMA ENABLE upon polling DMA control"
											 "register (HACK) as workaround");
		...
		case 0x89: // Channel volume register			
			if ( (curchan) && (!gus_Reg89_Silent) )
			{
				/*
					 Duke Nukem 1.5, Music Kaputt
				     RampFract mit 10 lässt nach einiger
					 Zeit alle möglichen Instrumente wahlos erklingen.
				*/
				
				return (Bit16u)((curchan->RampVol >> 8) << 8);
			}
			else return 0x0000;		
		...
		#if LOG_GUS
			LOG(LOG_MISC,LOG_NORMAL)("ULTRASND: Read Register num 0x%x", myGUS.gRegSelect);
		#endif
...
//	if (myGUS.gRegSelect|1!=0x44) LOG_MSG("write global register %x with %x", myGUS.gRegSelect, myGUS.gRegData);
	switch(myGUS.gRegSelect)
	{
		...
		case 0xE:  // Set active channel register
			...
			#if LOG_GUS
				LOG(LOG_MISC,LOG_NORMAL)("GUS set to %d channels fixed=%u freq=%luHz", myGUS.ActiveChannels,myGUS.fixed_sample_rate_output,(unsigned long)myGUS.basefreq);
			#endif
		...
		default:	
			#if LOG_GUS
				LOG(LOG_MISC,LOG_NORMAL)("ULTRASND: Unimplemented global register %x -- %x", myGUS.gRegSelect, myGUS.gRegData);
			#endif
			break;
	}
	return;
...
/* 
    ...
    I do not have a Gravis Ultrasound card with this type of mixer to test against.
...
public:
	...
	{
		#if LOG_GUS
			LOG(LOG_MISC,LOG_NORMAL)("GUS ICS-2101 Mixer Data Write val=%02xh to attensel=%u(%s) ctrlsel=%u(%s)",
									(int)val,
									addr_attenuator,
									attenuatorName(addr_attenuator),
									addr_control,
		...
		{ 
			/*	
				attenuator NTS:
				Only because an existing ICS patch for DOSBox does it this way... does hardware do it this way?
			*/
		...
		{ 	
			...
			*/
			...
			/* 
				FIXME:
				Does the panning take effect immediately, or does the chip require
				the DOS program to write attenuation again to apply the panning?
			*/
		}
		else
		{
...
public:
	...
	public:
		...
		void debugPrintMixer(const char *name) {
			LOG(LOG_MISC,LOG_NORMAL)("GUS ICS control '%s': %.3fdB %.3fdB",name,AttenDb[0],AttenDb[1]);
		}
	public:
		uint8_t		Panning;
		uint8_t		Control[2];
...
//		LOG(LOG_MISC,LOG_DEBUG)("GUS CS4231 write data addr=%02xh val=%02xh",addr,val);
		...
		switch (addr) {
			...
			default:
				#if LOG_GUS
					LOG(LOG_MISC,LOG_NORMAL)("GUS CS4231 unhandled data write addr=%02xh val=%02xh",addr,val);
				#endif
				break;
		}
	}
...
//		LOG(LOG_MISC,LOG_DEBUG)("GUS CS4231 read data addr=%02xh",addr);
		...
		switch (addr) {
			...
			default:
				#if LOG_GUS			
					LOG(LOG_MISC,LOG_NORMAL)("GUS CS4231 unhandled data read addr=%02xh",addr);
				#endif				
				break;
		}
	...
	void playio_data_write(uint8_t val) {
		LOG(LOG_MISC,LOG_NORMAL)("GUS CS4231 Playback I/O write %02xh",val);
	}
	uint8_t capio_data_read(void) {
		LOG(LOG_MISC,LOG_NORMAL)("GUS CS4231 Capture I/O read");
		return 0;
	}
	uint8_t status_read(void) {
		LOG(LOG_MISC,LOG_NORMAL)("GUS CS4231 Status read");
		return 0;
	}
...
//		LOG(LOG_MISC,LOG_DEBUG)("GUS CS4231 write reg=%u val=%02xh",reg,val);
		...
		switch (reg) {
			...
			case 0x2: /* Status Register (R2) */
				LOG(LOG_MISC,LOG_NORMAL)("GUS CS4231 attempted write to status register val=%02xh",val);
				break;
...
#if LOG_GUS	
		/*
		LOG(LOG_MISC,LOG_NORMAL)("Write gus port %x val %x",port,val);
		*/
#endif		
	switch(port - GUS_BASE) {
	case 0x200:
...
//TODO Check if 0x20a register is also available on the gus like on the interwave
	case 0x20b:
		...
		if (myGUS.gRegControl == 6) {
			 ...
			 * 0: reserved */
			/* TODO: */
			LOG(LOG_MISC,LOG_NORMAL)("GUS: DOS application wrote to 2XB register control number 0x06 (Jumper) val=%02xh",(int)val);
		}
		else if (myGUS.gRegControl == 5) {
			/* write a 0 to clear IRQs on power up (?) */
			LOG(LOG_MISC,LOG_NORMAL)("GUS: DOS application wrote to 2XB register control number 0x05 (Clear IRQs) val=%02xh",(int)val);
		}
			...
			if (myGUS.mixControl & 0x40) {
				...
				LOG(LOG_MISC,LOG_NORMAL)("GUS DMA reprogrammed: DMA1 %d, DMA2 %d",(int)myGUS.dma1,(int)myGUS.dma2);
...
void GUS_DMA_Event_Transfer(DmaChannel *chan,Bitu dmawords) {
	...
	#if LOG_GUS	
		LOG(LOG_MISC,LOG_NORMAL)("GUS DMA transfer %lu bytes, GUS RAM address 0x%lx %u-bit DMA %u-bit PCM (ctrl=0x%02x) tcount=%u",
			(unsigned long)step,
			(unsigned long)dmaaddr,
			(myGUS.DMAControl & 0x4 ) ? 16 : 8,
			(myGUS.DMAControl & 0x40) ? 16 : 8,
			myGUS.DMAControl,chan->tcount);
	#endif
	...
	{
		#if LOG_GUS	
			LOG(LOG_MISC,LOG_NORMAL)("GUS DMA transfer hit Terminal Count, setting DMA TC IRQ pending");
		#endif
...
void GUS_DMA_Event(Bitu val)
{
	...
	{
		#if LOG_GUS
			LOG(LOG_MISC,LOG_NORMAL)("GUS DMA event: DMA channel no longer exists, stopping DMA transfer events");
		#endif
	...
	{
		#if LOG_GUS		
			LOG(LOG_MISC,LOG_NORMAL)("GUS: Stopping DMA transfer interval, DMA masked=%u",chan->masked?1:0);
		#endif
		GUS_DMA_Active = false;
		return;
	}
	...
	{
		#if LOG_GUS		
			LOG(LOG_MISC,LOG_NORMAL)("GUS DMA event: DMA control 'enable DMA' bit was reset, stopping DMA transfer events");
		#endif
	...
	#if LOG_GUS	
		LOG(LOG_MISC,LOG_NORMAL)("GUS DMA event: max %u DMA words. DMA: tc=%u mask=%u cnt=%u",		
...
void GUS_StopDMA()
{
	if (GUS_DMA_Active)
		#if LOG_GUS
			LOG(LOG_MISC,LOG_NORMAL)("GUS: Stopping DMA transfer interval");
		#endif
...
void GUS_StartDMA()
{
	if (!GUS_DMA_Active)
	{
		...
		#if LOG_GUS
			LOG(LOG_MISC,LOG_NORMAL)("GUS: Starting DMA transfer interval");
		#endif
		...
		{
            LOG(LOG_MISC,LOG_WARN)("GUS warning: Both DMA channels set to the same channel WITHOUT"
								   "combining! This is documented to cause bus conflicts on real hardware");
		}
    }	
}
...
static void GUS_DMA_Callback(DmaChannel * chan,DMAEvent event)
{
	...
	{
		#if LOG_GUS
			LOG(LOG_MISC,LOG_NORMAL)("GUS: DMA unmasked");
		#endif
	...
	{
		#if LOG_GUS		
			LOG(LOG_MISC,LOG_NORMAL)("GUS: DMA masked. Perhaps it will stop the DMA transfer event.");
		#endif
	}
}
...
static void MakeTables(void) {
	...
	if (gus_fixed_table) {
		for (i=0;i < 16;i++)
			...
			LOG(LOG_MISC,LOG_NORMAL)("GUS: using accurate (fixed) pantable");
	}
	else {
		for (i=0;i < 8;i++)
			pantable[i] = 0;
		...
		LOG(LOG_MISC,LOG_NORMAL)("GUS: using old (naive) pantable");
		
	}
	
		/*
		LOG(LOG_MISC,LOG_NORMAL)("GUS pantable (attenuation, left to right in dB): hard left -%.3f, -%.3f, -%.3f, -%.3f, -%.3f, -%.3f, -%.3f, center(7) -%.3f, center(8) -%.3f, -%.3f, -%.3f, -%.3f, -%.3f, -%.3f, -%.3f, hard right -%.3f",
...
public:
	GUS(Section* configuration):Module_base(configuration){
		...
		myGUS.clearTCIfPollingIRQStatus = section->Get_bool("ClearTcIRQ");
		if (myGUS.clearTCIfPollingIRQStatus) {
			LOG(LOG_MISC,LOG_NORMAL)("GUS: Will clear DMA TC IRQ if excess polling, as instructed");
		}
		...
		myGUS.initUnmaskDMA = section->Get_bool("UnMaskDMA");
		if (myGUS.initUnmaskDMA){
			LOG(LOG_MISC,LOG_NORMAL)("GUS: Unmasking DMA at boot time as requested");
		}
			
		myGUS.fixed_sample_rate_output = section->Get_bool("fixrate"); 
		/*
			LOG("UltraSound: Using %s Sample Rate Output",myGUS.fixed_sample_rate_output?"Fixed":"Realistic");
		*/
		
		myGUS.force_master_irq_enable=section->Get_bool("ForceMIRQ");
		if (myGUS.force_master_irq_enable){
			LOG(LOG_MISC,LOG_NORMAL)("GUS: Master IRQ enable will be forced on as instructed");
		}
		
		unmask_irq		= section->Get_bool("UnMaskIRQ");		
        enable_autoamp 	= section->Get_bool("AutoAmp");	
        startup_ultrinit= section->Get_bool("ultrinit");
		...
		int x = section->Get_int("MemorySize");
		if (x >= 0){
		...
		if ((myGUS.memsize&((256u << 10u) - 1u)) != 0) {
			LOG_MSG("UltraSound: Warning - %uKB onboard is an unusual value. Usually GUS cards have some multiple of 256KB RAM onboard",myGUS.memsize>>10);
		}
		
		LOG_MSG("UltraSound: Have %uKB onboard",myGUS.memsize>>10);		
        ...
        myGUS.masterVolume = section->Get_double("GusVolume");
    ...
    //void DOS_Startup() {		
		...
		*/
		...
		if ( startup_ultrinit == true ){
			autoexecline[3].Install(std::string("@ECHO    UltraSound Initialization."));
		}		
