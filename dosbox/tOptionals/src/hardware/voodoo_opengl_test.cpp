...
UINT32 texrgb[1024*1024];// [256 * 256] ;
...
static INLINE float ScaleFastFillY(GLsizei h, GLint y) {
	for (int cy = ((y - 1) * ((double)h / (double)v->fbi.height) + (h / v->fbi.height)); cy <= (y * ((double)h / (double)v->fbi.height)); cy++)
	{
		return cy;
	}
	return 0;
}
INLINE float ScaleFactorY(GLsizei h, GLint y) {
	for (int cy = ((y - 1) * ((double)h / (double)v->fbi.height)); cy <= (y * ((double)h / (double)v->fbi.height)) - ((double)h / (double)v->fbi.height); cy++)
	{
		return cy;
	}
	return 0;
}
INLINE float ScaleFactorX(GLsizei w, GLint x) {
	for (int cx = ((x - 1) * ((double)w / (double)v->fbi.width) + ((double)w / (double)v->fbi.width)); cx <= (x * ((double)w / (double)v->fbi.width)); cx++)
	{
		return cx;
		break;
	}
...
INLINE void ogl_get_vertex_data(INT32 x, INT32 y, const void *extradata, ogl_vertex_data *vd) {
...
//			lod += DITHER4[(XX) & 3] << 4;
	...
	*/
	...
	//LOG(LOG_FXOGL, LOG_NORMAL)("[%d] Get Vertex Data [%dx%d] VD->[%fx%f]", __LINE__, x, y, vd->x, vd->y);
...
void voodoo_ogl_draw_pixel(int x, int y, bool has_rgb, bool has_alpha, int r, int g, int b, int a) {
	...
	//if (bUseTexDrawInfo) { LOG_MSG("OGL Voodoo_Ogl_Draw_Pixel In Use (" __FILE__ ") :%d", __LINE__); }
	...
	if (LFBMODE_WRITE_BUFFER_SELECT(v->reg[lfbMode].u)==0) {
		//if (bUseTexDrawInfo) { LOG_MSG("   Ogl Draw Pixel: VOGL_SetDrawMode(true) (" __FILE__ ") :%d", __LINE__); }
					
		VOGL_SetDrawMode(true);
			v->fbi.vblank_flush_pending=true;			
	} else {
		//if (bUseTexDrawInfo) { LOG_MSG("   Ogl Draw Pixel: VOGL_SetDrawMode(false) (" __FILE__ ") :%d", __LINE__); }
		VOGL_SetDrawMode(false);
	}
	...
	//if (bUseTexDrawInfo) { LOG_MSG("   Ogl Draw Pixel: [x2]x[y2] = [%d]x[%d] (" __FILE__ ") :%d", x2,y2, __LINE__); }
...
void voodoo_ogl_draw_z(int x, int y, int z) {
	//LOG(LOG_VOODOO,LOG_WARN)("[x %d, y %d]",x,y,__FILE__,__LINE__);
	VOGL_ClearBeginMode();
...
/*///////////////////////////////////////////////////////////////////////////////////////////////////////////////////*/
static INLINE UINT32* Voodoo_OGL_Read_GlPix(GLint X, GLint Y, GLsizei Width, GLsizei Height, GLenum Format, GLenum Type, UINT32* data){
	glReadPixels(X,Y,Width,Height,Format,Type ,data);
	return data;
};

/*///////////////////////////////////////////////////////////////////////////////////////////////////////////////////*/
static INLINE UINT32* Voodoo_OGL_Read_Clear(UINT32* nMemory) {
	
	UINT32* buffer = nMemory;
	if (buffer != NULL)free(buffer);
	return buffer;
};

/*///////////////////////////////////////////////////////////////////////////////////////////////////////////////////*/
static INLINE UINT32 Voodoo_OGL_Read_Back(GLint X, GLint Y, GLsizei Width, GLsizei Height, GLenum Format){
	...
	if ((Cache.Line.Back.Y != Y) || (X + 1 >= Cache.Line.Back.W)) {
		...
		for (Bitu i = 0; i <= 1; i++)
		{
			if (&Cache.Line.Back.Buffer[i] != 0)
			{
				VOGL_SetReadMode(true);
				Voodoo_OGL_Read_GlPix(0, Height - Y, Width, 1, Format, GL_UNSIGNED_INT_8_8_8_8_REV, &Cache.Line.Back.Buffer[i]);
			}
		}
...
/*///////////////////////////////////////////////////////////////////////////////////////////////////////////////////*/
static INLINE UINT32 Voodoo_OGL_Read_Front(GLint X, GLint Y, GLsizei Width, GLsizei Height, GLenum Format){
	...
	if ((Cache.Line.Frnt.Y != Y) || (X + 1 >= Cache.Line.Frnt.W)) {
		...
		for (Bitu i = 0; i <= 1; i++)
		{
			if (&Cache.Line.Frnt.Buffer[i] != 0)
			{
				VOGL_SetReadMode(true);
				Voodoo_OGL_Read_GlPix(0, Height - Y, Width, 1, Format, GL_UNSIGNED_BYTE, &Cache.Line.Frnt.Buffer[i]);
			}
		}
...
/*///////////////////////////////////////////////////////////////////////////////////////////////////////////////////*/
static INLINE float ScaleAspectRatioX(GLsizei w, GLint x)
{
...
static INLINE float ScaleAspectRatioW(GLsizei w)
{
	if ((sdlVoodoo.opengl.voodoo_aspect == true) && (sdlVoodoo.fullscreen))
	{
		return w = w - ((w / 2) / 2);
