...
INLINE void Get_Vertx_Data_Iter(const poly_extra_data* extra, uint32_t x, uint32_t y)
{
	VertexData.VertX/*dx*/ = x - extra->ax;
	VertexData.VertY/*dy*/ = y - extra->ay;

	VertexData.iterR = extra->startr + ((VertexData.VertY/*dy*/ * extra->drdy) >> 4) + ((VertexData.VertX/*dx*/ * extra->drdx) >> 4);
	VertexData.iterG = extra->startg + ((VertexData.VertY/*dy*/ * extra->dgdy) >> 4) + ((VertexData.VertX/*dx*/ * extra->dgdx) >> 4);
	VertexData.iterB = extra->startb + ((VertexData.VertY/*dy*/ * extra->dbdy) >> 4) + ((VertexData.VertX/*dx*/ * extra->dbdx) >> 4);
	VertexData.iterA = extra->starta + ((VertexData.VertY/*dy*/ * extra->dady) >> 4) + ((VertexData.VertX/*dx*/ * extra->dadx) >> 4);
	VertexData.iterZ = extra->startz + ((VertexData.VertY/*dy*/ * extra->dzdy) >> 4) + ((VertexData.VertX/*dx*/ * extra->dzdx) >> 4);
	VertexData.iterW = extra->startw + ((VertexData.VertY/*dy*/ * extra->dwdy) >> 4) + ((VertexData.VertX/*dx*/ * extra->dwdx) >> 4);
}
void ogl_get_vertex_data(uint32_t x, uint32_t y, const void *extradata, ogl_vertex_data *vd) {
	...
	*/
	...
	Get_Vertx_Data_Iter(extra,x,y);

	for (auto i=0; i<extra->texcount; i++) {
		...
		iterw = v->tmu[i].startw + ((VertexData.VertY/*dy*/ * v->tmu[i].dwdy)>>4) + ((VertexData.VertX/*dx*/ * v->tmu[i].dwdx)>>4);
		iters = v->tmu[i].starts + ((VertexData.VertY/*dy*/ * v->tmu[i].dsdy)>>4) + ((VertexData.VertX/*dx*/ * v->tmu[i].dsdx)>>4);
		itert = v->tmu[i].startt + ((VertexData.VertY/*dy*/ * v->tmu[i].dtdy)>>4) + ((VertexData.VertX/*dx*/ * v->tmu[i].dtdx)>>4);
...
//			lod += DITHER4[(XX) & 3] << 4;
	...
	vd->r = (float)( (float)VertexData.iterR/*iterr*/ /(float)(1<<20) );
	vd->g = (float)( (float)VertexData.iterG/*iterg*/ /(float)(1<<20) );
	vd->b = (float)( (float)VertexData.iterB/*iterb*/ /(float)(1<<20) );
	vd->a = (float)( (float)VertexData.iterA/*itera*/ /(float)(1<<20) );
	vd->z = (float)( (float)VertexData.iterZ/*iterz*/ /(float)(1<<20) );
	vd->w = (float)( (float)VertexData.iterW/*iterw*/ /(float)(0xffffff));

	INT32 wfloat;
	ogl_get_depth(v, VertexData.iterZ/*iterz*/, VertexData.iterW/*iterw*/,&VertexData.d,&wfloat);
	vd->d = (float)((float)VertexData.d/(float)(0xffff));

	INT32 fogblend;
	ogl_get_fog_blend(v,wfloat, VertexData.iterZ/*iterz*/, VertexData.iterW/*iterw*/,&fogblend);
...
INLINE void Cache_Texture_GetID(int format, int j, GLuint texID, uint32_t smax, uint32_t tmax, uint32_t offset, uint32_t*texrgbp)
{
	switch (format)
	{
		case 1:
		...
		{				
			for (int i = 0; i < (smax * tmax); i++) {
				UINT8* texptr8 = (UINT8*)&v->tmu[j].ram[(offset)&v->tmu[j].mask];
			...
			#if defined(C_DEBUG)
					LOG(LOG_FXTEXFORMAT, LOG_NORMAL)("[%d]Texture ID:%d Format:%d [SMax=%d x TMax=%d]", __LINE__, texID, format, smax, tmax);
			#endif
		}
		return;
		case 5:
		...
		{
			for (int i = 0; i < (smax * tmax); i++) {
				UINT16* texptr16 = (UINT16*)&v->tmu[j].ram[(offset)&v->tmu[j].mask];
		...
		{
			#if defined(C_DEBUG)
					LOG(LOG_FXTEXFORMAT, LOG_NORMAL)("[%d]Texture ID:%d Format:%d [SMax=%d x TMax=%d] (Unknown)", __LINE__, texID, format, smax, tmax);
			#endif
		}
	}

}
INLINE void Cache_Texture_Filter(uint32_t TEXMODE)
{
		...
		if ( extra->texcount && (extra->texcount >= j) && (v->tmu[j].lodmin < (8 << 8)) ) {
			...
			if (t != textures[j].end())
			{
				...
				else
				{
					...
					std::map<const UINT32, GLuint>::iterator u = t->second.ids->find( calculate_palsum(j)/*psum*/);
					if (u != t->second.ids->end()) {
						...
						{						
							//LOG(LOG_FXOGL, LOG_NORMAL)("[%d]Texture Removed ID:%d Format:%d [Size=%d]", __LINE__, texID, t->second.ids->size());
							std::map<const uint_fast32_t/*UINT32*/, GLuint>::iterator u;
			...
			if (!valid_texid) {
				...
				Cache_Texture_GetID(format, j, texID, smax, tmax, texboffset, (UINT32*)&texrgb[0]);
				/*
				switch (format)			
				{			
					case 1:
				...
				*/
				...
				glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, smax, tmax, 0, GL_BGRA_EXT, GL_UNSIGNED_BYTE, (const GLvoid*)texrgbp);
...
void voodoo_ogl_invalidate_paltex(void) {
	...
	for (auto j=0; j<2; j++)
	{
		for (auto t = textures[j].begin(); t != textures[j].end(); t++)
		{
...
INLINE void DrawTriangle_VertexLoop(ogl_texture_data td[2], ogl_vertex_data vd[3], poly_extra_data* extra)
{
	...
	for (auto /*unsigned int*/ i = 0; i < 3; i++)
	{
		glColor4fv(&vd[i].r);

		for (auto /*unsigned int*/ t = 0; t < 2; t++)
			if (td[t].enable)
			{
...
void voodoo_ogl_draw_pixel(int x, int y, bool has_rgb, bool has_alpha, int r, int g, int b, int a) {
	...
	if (sdlVoodoo.opengl.gl_QuadsDraw == true) {
		...
		VOGL_BeginMode(GL_QUADS);
		{
			...
			glTexCoord2f(w, 0);
			glVertex2f(x + (GLfloat)1, y);

			glTexCoord2f(w, h);
			glVertex2f(x + (GLfloat)1, y - (GLfloat)1);

			glTexCoord2f(0, h);
			glVertex2f(x, y - (GLfloat)1);
		}

	}
	else {
		GLfloat x2 = (GLfloat)x; +0.5;
		GLfloat y2 = (GLfloat)y; +0.5;
	...
	if (sdlVoodoo.opengl.gl_QuadsDraw == true) {
		...
		VOGL_BeginMode(GL_QUADS);
		{
			...
			*/
			...
			glTexCoord3f(w, 0, 0);
			glVertex3f(x + (GLfloat)1, y, z);

			glTexCoord3f(w, h, 0);
			glVertex3f(x + (GLfloat)1, y - (GLfloat)1, z);

			glTexCoord3f(0, h, 0);
			glVertex3f(x, y - (GLfloat)1, z);
		}
	}
	else
	{
	...
	if (sdlVoodoo.opengl.gl_QuadsDraw == true) {
		...
		VOGL_BeginMode(GL_QUADS);
		{
			...
			*/
			...
			glColor4ub((GLubyte)(r & 0xff), (GLubyte)(g & 0xff), (GLubyte)(b & 0xff), (GLubyte)(0xff));
			glTexCoord2f(0, 0);
			glVertex2f(x, y);
...
void voodoo_ogl_fastfill(void) {
	...
	bool scissors_needed = true; sdlVoodoo.opengl.glScissor_flag;
	if ((sx == 0) && (sy == 0))
	{
...
INLINE UINT32* Voodoo_OGL_Read_GlPix(GLint X, GLint Y, GLsizei Width, GLsizei Height, GLenum Format, GLenum Type, int Side,int Place, UINT32* data){	
	...
	{		
		if (Voodoo_OGL_Buffer_GetColor(Cache.Line.Back.Buffer[0], Side, Place) == true)
		{
			...
			glReadPixels(X, Y, Width, Height, Format, Type, &Cache.Line.Back.Buffer[0]);
		}
	...
	} else if ((Side == 1) && (Place == 1))
	{
		if (Voodoo_OGL_Buffer_GetColor(Cache.Line.Back.Buffer[1], Side, Place) == true)
		{
			Format	= Voodoo_OGL_Buffer_GetFormat();
			Type	= Voodoo_OGL_Buffer_GetType();
			glReadPixels(X, Y, Width, Height, Format, Type, &Cache.Line.Back.Buffer[1]);
		}
	} else if ((Side == 0) && (Place == 0))
	{
		if (Voodoo_OGL_Buffer_GetColor(Cache.Line.Frnt.Buffer[0], Side, Place) == true)
		{
			Format	= Voodoo_OGL_Buffer_GetFormat();
			Type	= Voodoo_OGL_Buffer_GetType();
			glReadPixels(X, Y, Width, Height, Format, Type, &Cache.Line.Frnt.Buffer[0]);
		}
	} else if ((Side == 0) && (Place == 1))
	{
		if (Voodoo_OGL_Buffer_GetColor(Cache.Line.Frnt.Buffer[1], Side, Place) == true)
		{
			Format	= Voodoo_OGL_Buffer_GetFormat();
			Type	= Voodoo_OGL_Buffer_GetType();
			glReadPixels(X, Y, Width, Height, Format, Type, &Cache.Line.Frnt.Buffer[1]);
		}
		...
		if (sdlVoodoo.fullscreen){
			if (sdlVoodoo.ScrOpenGL_Flags & SDL_WINDOW_FULLSCREEN_DESKTOP){
				...
				*/
				...
				adjust_x = (displayMode.w - (GLsizei)sdlVoodoo.pciFSW) /2;
				adjust_y = (displayMode.h - (GLsizei)sdlVoodoo.pciFSH) /2;						
...
void vPCI_Get_Configuration(void){
	...
	sdlVoodoo.opengl.GL_filtering=1;
	if (!_stricmp(sdlVoodoo.opengl.sfiltering,"none"))
	{
		sdlVoodoo.opengl.GL_filtering=0;
		
	}else if (!_stricmp(sdlVoodoo.opengl.sfiltering,"point"))
	{
		sdlVoodoo.opengl.GL_filtering=1;
		
	}else if (!_stricmp(sdlVoodoo.opengl.sfiltering,"bilinear"))
	{
		sdlVoodoo.opengl.GL_filtering=2;
	
	}else if (!_stricmp(sdlVoodoo.opengl.sfiltering,"trilinear"))
	{
		sdlVoodoo.opengl.GL_filtering=3;
		
	}else if (!_stricmp(sdlVoodoo.opengl.sfiltering,"anisotropic"))
	{
		sdlVoodoo.opengl.GL_filtering=4;
	
	}else if (!_stricmp(sdlVoodoo.opengl.sfiltering,"testmode"))
	{
		sdlVoodoo.opengl.GL_filtering=5;
	}	
	...
	*/
	...
	if (!_stricmp(sdlVoodoo.opengl.sglshademdl,"none"))
	{
		sdlVoodoo.opengl.GL_ShadeModel=0;
		
	}else if (!_stricmp(sdlVoodoo.opengl.sglshademdl,"flat"))
	{
		sdlVoodoo.opengl.GL_ShadeModel=1;
		
	}else if (!_stricmp(sdlVoodoo.opengl.sglshademdl,"smooth"))
	{
		sdlVoodoo.opengl.GL_ShadeModel=2;
	
	}		

	sdlVoodoo.opengl.gl_wrap_s=10497;
	if (!_stricmp(sdlVoodoo.opengl.sgl_wrap_s,"gl_repeat"))
	{
		sdlVoodoo.opengl.gl_wrap_s=10497;
		
	}else if (!_stricmp(sdlVoodoo.opengl.sgl_wrap_s,"gl_mirrored_repeat"))
	{
		sdlVoodoo.opengl.gl_wrap_s=33648;
		
	}else if (!_stricmp(sdlVoodoo.opengl.sgl_wrap_s,"gl_clamp_to_border"))
	{
		sdlVoodoo.opengl.gl_wrap_s=33069;
	
	}else if (!_stricmp(sdlVoodoo.opengl.sgl_wrap_s,"gl_clamp_to_edge"))
	{
		sdlVoodoo.opengl.gl_wrap_s=33071;
	
	}	
	
	sdlVoodoo.opengl.gl_wrap_t=10497;
	if (!_stricmp(sdlVoodoo.opengl.sgl_wrap_t,"gl_repeat"))
	{
		sdlVoodoo.opengl.gl_wrap_t=10497;
		
	}else if (!_stricmp(sdlVoodoo.opengl.sgl_wrap_t,"gl_mirrored_repeat"))
	{
		sdlVoodoo.opengl.gl_wrap_t=33648;
		
	}else if (!_stricmp(sdlVoodoo.opengl.sgl_wrap_t,"gl_clamp_to_border"))
	{
		sdlVoodoo.opengl.gl_wrap_t=33069;
	
	}else if (!_stricmp(sdlVoodoo.opengl.sgl_wrap_t,"gl_clamp_to_edge"))
	{
		sdlVoodoo.opengl.gl_wrap_t=33071;
	
	}	
	
	sdlVoodoo.opengl.gl_ortho_zNear		= 0.0f;		sdlVoodoo.opengl.gl_ortho_zFar		= -1.0f;
	sdlVoodoo.opengl.glZoomFaktor_W		= 0;		sdlVoodoo.opengl.glZoomFaktor_H		= 0;
	sdlVoodoo.opengl.gl_Major_Version 	= 2;		sdlVoodoo.opengl.gl_Minor_Version 	= 1;
	...
	sdlVoodoo.opengl.glZoomFaktor_W		=  section->Get_int("ZoomScreen_Width");
	sdlVoodoo.opengl.glZoomFaktor_H		=  section->Get_int("ZoomScreen_Height");	
